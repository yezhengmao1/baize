...
|---|---|---|
| `gpu-flame` | Step-boundary detector + optional flame graph. Detects step boundaries from the kernel->NVTX mapping (`--step-nvtx SUBSTR`, default `Optimizer.step`), skips warmup (`--skip-steps N`, default 1), prints the per-step window table. With `--flamegraph OUT` builds an interactive HTML flame graph (`<OUT>.html`, d3-flame-graph: **Sum** widths = Σ kernel GPU time, **Solo** widths = Σ per-kernel solo time / `active_count==1`; sum→solo shrink = overlapped/hidden, equal = exposed; tooltip sum/solo/overlap%); `--stack-depth N` truncates the CPU NVTX path. | `gpu-flame <db.sqlite> [--step-nvtx SUBSTR] [--skip-steps N] [--stack-depth N] [--flamegraph OUT]` |
